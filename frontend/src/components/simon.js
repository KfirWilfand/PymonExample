  ...
  activateBtn(color, userInitiated) {
    this.setState(
      ...
      () => {
        ...
        setTimeout(() => {
          this.setState({ activeBtn: "none" });
        }, 500);
      }
    );
    if (userInitiated) {
      ajax(`/games/${getGameId()}/turn`, {
        method: "POST",
      ...
      <div className="simon">
        {BUTTONS.map(b => (
          <SimonBtn
            key={b}
            color={b}
            active={this.state.activeBtn == b}
            disabled={this.props.disabled}
            clickAction={this.activateBtn.bind(this)}
          />
        ))}
        {this.state.sequenceStep == 0 && this.props.showPlayBtn && (
          <button className="play-btn" onClick={this.playSequence.bind(this)}>
            Play
          </button>
        )}
      </div>
